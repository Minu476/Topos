...
**M10 was approved and implemented 2026-07-27** — see `docs/DECISIONS.md`'s "M10 APPROVED AND
IMPLEMENTED" entry. It extends the M0–M9 structure the same way M9 did; nothing here reopens
M0–M9.

**M11 phase 1 was approved and implemented 2026-07-30** — see `docs/DECISIONS.md`'s "M11 PHASE 1
APPROVED AND IMPLEMENTED" entry. `docs/ALGORITHM_SPEC.md` proposed six must-have algorithms (of
which three — Directed SCC, Centrality, PageRank — need only the existing GDS oracle; the other
three — s-connected-components, s-line-graph, s-diameter — need a new HNX oracle project not yet
built). Phase 1 ships the GDS-only three; the s-walk family is phase 2, not yet scheduled.

**M11 phase 1's live-GDS spot-check, in-repo consumer, and docs landed 2026-08-03** — see
`docs/DECISIONS.md`'s same-dated entry. All 6 GDS-oracle parity tests now run against a real
Neo4j+GDS instance (Closeness tightened to exact-value; PageRank's comparison fixed to L1-normalize
GDS's non-probability-distribution default before comparing — a real convention difference between
the two implementations, not a bug). `samples/Topos.Samples.ChatMemory` became a real in-repo
consumer of all three algorithms; `API_REFERENCE.md`/`USAGE_PATTERNS.md` gained the corresponding
entries. The cross-repo half of M11's exit criterion (RLB/NexusVerifier adoption) remains open.
...
| **M9** | Layer-1 role-aware directed traversal (`Topos.Hypergraph.Knowledge` package, added 2026-07-25, **implemented 2026-07-26**) | `DirectedBfs`/`DirectedShortestPath`/`RoleFilteredMembers` over `IHypergraphQuery`, generalizing the identical pattern three independent consumers already hand-rolled (`ChatMemory.EntitiesMentionedIn`; RLB's `ToposGraphProjection.DirectedBfs`/`DirectedShortestPath`, already RLB-agnostic; NexusVerifier's `CandidateEdgesFor`/`SubgoalsOf`). No kernel changes — pure consumer of the existing public surface. Also hosts `docs/ROLE_CONVENTIONS.md`'s byte-backed-enum pattern as real code (`AddIncidence<TRole>`). | **🔒 MET (2026-07-26):** RLB's `ToposGraphProjection` refactored to call `Topos.Hypergraph.Knowledge.DirectedTraversal` instead of its own hand-rolled copy (three private methods deleted); RLB's 346-test suite passes unchanged. See `docs/DECISIONS.md`'s "M9 IMPLEMENTED" entry. |
| **M10** | `Topos.Hypergraph.Mcp` server (proposed 2026-07-26, **implemented 2026-07-27**) — expose the kernel's and `Topos.Hypergraph.Knowledge`'s public API as 18 Model Context Protocol tools, built on Microsoft's official `ModelContextProtocol` C# SDK (v1.4.1, Apache-2.0 — the spec's own "MIT" claim was wrong, corrected during this pass). Stateful single-session, stdio transport, opaque-string Handles, tagged-union property values — all four §5 forks that gate v1 scope resolved to the spec's tentative leans, confirmed explicitly by Nasser rather than silently adopted. No kernel changes — pure consumer. | `src/Topos.Hypergraph.Mcp/` (`ToposMcpServer.cs`, `TypeMapping.cs`, `Transport/StdioHost.cs`), 13 new tests in `tests/Topos.Hypergraph.Mcp.Tests/`, `samples/Topos.Samples.McpAgent/`. Deliberately not exposed: `RestoreVertex`, `AllIncidences`, `HasCycle`. See `docs/DECISIONS.md`'s "M10 APPROVED AND IMPLEMENTED" entry for the full build record. | **🔒 MET at the protocol level (2026-07-27):** real `initialize`/`tools/list`/`tools/call` JSON-RPC frames sent by hand over the actual stdio transport confirmed all 18 tools register with correct schemas and `create_vertex`→`add_incidence`→`is_reachable` behaves per the kernel's documented semantics. **Still open:** literal agent-in-the-loop dogfood (Claude Code restart needed to pick up the new `topos` entry in this repo's `.mcp.json`). |
| **M11 phase 1** | Algorithm completeness, GDS-only slice (proposed `docs/ALGORITHM_SPEC.md` 2026-07-30, **implemented 2026-07-30**, **live-verified + given an in-repo consumer + documented 2026-08-03**) — `Centrality` (degree/closeness/betweenness) and `PageRank` static classes in the kernel, both over the existing `BipartiteAdjacency` M6 shares (not M1's `GetBfs` adjacency — a deliberate deviation from the spec's literal phrasing, made so both reuse the existing `CliqueExpansionProjectionEngine` oracle with zero new projection machinery); `DirectedScc` (iterative Tarjan) added to `Topos.Hypergraph.Knowledge`'s `DirectedTraversal`, replacing the class of hand-rolled cycle guards M9's own doc named (NexusVerifier finding #4). No kernel-contract changes — pure additive static classes / extension methods. **Phase 2 (s-connected-components, s-line-graph, s-diameter) is not scheduled** — it needs a new `tests/Topos.Tests.HnxOracle` project (Docker sidecar, per spec §6.1) that doesn't exist yet. | `src/Topos.Hypergraph/Centrality.cs`, `src/Topos.Hypergraph/PageRank.cs`, `src/Topos.Hypergraph.Knowledge/DirectedTraversal.cs` (`DirectedScc` + typed-role overload in `RoleExtensions.cs`), 30 new unit tests (`CentralityTests`, `PageRankTests`, `DirectedSccTests`), 6 GDS-oracle parity tests (`CentralityGdsParityTests`, `PageRankGdsParityTests`, `DirectedSccGdsParityTests`), `samples/Topos.Samples.ChatMemory` consumer additions (`MostConnectedEntities`/`RankByImportance`/`RecordDerivation`+`DetectCircularDerivations`, 4 new tests), `API_REFERENCE.md`/`USAGE_PATTERNS.md` entries. See `docs/DECISIONS.md`'s "M11 PHASE 1 APPROVED AND IMPLEMENTED" and 2026-08-03 follow-up entries. | **🔒 MET for Degree/Betweenness/Closeness/PageRank/DirectedScc** — all 6 GDS-oracle parity tests now run exact-value against a real, live Neo4j+GDS instance (2026-08-03), including Closeness (tightened from a ranking-only assertion once the oracle was actually reachable) and PageRank (fixed to L1-normalize GDS's non-probability-distribution default before comparing — a real convention difference between the implementations, not a bug); hand-computed golden unit tests pass (233/233 full suite, up from 177). **In-repo consumer:** `samples/Topos.Samples.ChatMemory` uses all three algorithms for real (ranking entities, ranking importance, detecting circular fact derivation). **Not yet met:** the cross-repo half of M11's own exit criterion (spec §7) — no RLB/NexusVerifier consumer has adopted the new surface yet; DirectedScc replacing NexusVerifier's hand-rolled guard is the natural next forcing-function step, in that repo. |
