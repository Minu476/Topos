...
`[verified:src=/Users/nassertowfigh/Projects/Patents/Patent 5 Filing copy/patent_5_draft.md]`
...
embeddings) and held in all four architecturally distinct models. `[verified:src=patent_5_draft.md
FIG. 7, Replications section]`
...
**One thing worth flagging to Nasser directly, outside this doc's scope:** Patent 5 is an
unfiled/in-progress patent draft. This search read it only to resolve the citation question; no
patent content is reproduced here beyond what's needed to identify Claim 3 and explain the
conflation.

## Search method

`rg -il "paradox"` across all sibling directories under `~/Projects` (not just RLB), excluding
`.git`/`bin`/`obj`/`node_modules`. Hits fell into three groups: (1) the real artifact above
(`Patents/`), (2) unrelated uses of the word "paradox" as a puzzle-domain tag in RLB/RLRL-LLM's
`ReasoningProblemSet.cs` ("logic-jailer-paradox", "sci-capacitor-paradox" — LLM reasoning
benchmark prompts, not a compression finding), and (3) Hash-X's own "beautiful-ugly composition"
work, which is the *source* research Patent 5 formalizes — same project family, confirms rather
than complicates the Patent-5 attribution.
